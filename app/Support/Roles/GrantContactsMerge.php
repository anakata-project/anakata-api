...
final class GrantContactsMerge
{
    public const REASON = 'Sprint 9: Manager may merge CRM contacts';
    ...
    public static function grant(): void
    {
        DB::transaction(function (): void {
            ...
            if ($role->permissions->contains(Permission::ContactsMerge)) {
                return;
            }

            $before = self::sortedValues($role);
            $role->permissions = $role->permissions->push(Permission::ContactsMerge)->unique()->values();
            $role->save();
            ...
            History::record(
                ...
                after: [
                    'permissions' => $after,
                    'added' => [Permission::ContactsMerge->value],
                    'removed' => [],
                ],
                reason: self::REASON,
                actor: null,
