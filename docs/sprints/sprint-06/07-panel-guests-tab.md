...
4. **Guest cards** (prototype `guestCard`): name or "Guest {n} — name pending", LEAD and MINOR pills, age at departure, country name, EC resident, the passport **as the API sent it** (masked or full), expiry, PNG category label and fee, insurance declared or not, guardian status for minors, and "medical note on file" when `medical_note.on_file` is true and `medical_note.value` is null. The three notes arrive as `{ value, on_file }` objects (task 01's `Masking::note`); `value` is null whenever the user may not see it. Incomplete cards take the prototype's `inc` style.
5. **Edit form** (prototype `guestForm`), one guest at a time:
   - names, DOB, nationality (a select filled from `GET /api/rms/countries`, added in task 06's prelude; no copy of `COUNTRIES` in the panel), EC resident, passport number, expiry, email, insurance declared.
   ...
   - **Medical, dietary and accessibility notes** only for `guests.view_sensitive`; the form reads and writes each note's `value`.
