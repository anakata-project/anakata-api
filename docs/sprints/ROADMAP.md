...
|---|---|---|
| **0** | Docker extension, two databases + two MySQL users, packages, module skeleton, health, CORS/Sanctum config, CI | anakata-ui: tokens, Nuxt UI theme, fonts, base `Ank*` components, style guide · panel: shell with RMS ⇄ CRM switch, both navigations, placeholders · engine: shell |
...
| **6** | Guests (encrypted), consents log, guardian consent, PNG/TCT fees + collection choice, extras catalogue + booking extras | panel · RMS: Guests tab, Extras tab, Contacts In |
