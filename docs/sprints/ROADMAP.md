...
| **3** | Yachts, cabins, itineraries, departures (generate season, Sunday-only, locks), internal blocks, **computed availability** | panel · RMS: Calendar, Yacht Layout, Itineraries, Departures, Internal Blocks |
| **4** | Bookings, state machine, **pricing engine** (doc 02 + dev decision B2), groups, booking requests, date change, deleted/released audit, holds, waitlist | panel · RMS: Bookings list + Groups, booking panel (Overview, History), New Reservation, Booking Requests, Holds & Waitlist |
...
| **9** | CRM core: contacts, identity resolution + aliases, CRM reads booking data directly (no event consumers or mirrors), attribution written once, behavioural events | panel · CRM: contacts, timeline, Web & Engine Activity, Sync & Field Ownership |
| **10** | CRM pipeline (stages 5–7 locked), tasks & SLA, consent register + subject requests, campaigns that read RMS offers/bookings directly, documents & delivery | panel · CRM: Pipeline & Forecast, Tasks & SLA, Consent & Data Rights, Campaigns & Offers, Documents & Delivery |
