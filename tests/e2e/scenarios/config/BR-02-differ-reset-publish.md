...
## Expected
- [ ] E1 · At 15 (before reset), the FIN-005 row shows `≠ differs from source`. **Differs / flagged** KPI is **7** (6 seeded flags + this differ).
- [ ] E2 · `Reset to source` returns the value to `12`, the differs mark is gone, KPI is **6**, and `Reset to source` is no longer offered on that row.
