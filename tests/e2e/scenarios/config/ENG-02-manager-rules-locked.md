...
- [ ] E3 · Rule panels (**Guests & capacity**, **Sales calendar & search**, fees) show the `ADMIN` pill; copy panels show `ADMIN · MANAGER`. **Private charter page** is one panel with pill `ADMIN · MANAGER`; only `Response SLA (hours)` is locked, copy fields on that panel stay enabled.
