...
- [ ] E2 · Jobs include `inventory:release-expired-holds` and `engine:expire-stripe-checkouts` (`* * * * *`); `anakata:crm-tasks` and `anakata:alerts` (`*/5 * * * *`); `anakata:flag-overdue`, `anakata:retention`, `anakata:events-retention`, `anakata:documents-due` (`0 0 * * *` · `Pacific/Galapagos`). Last run / outcome `—`. Next run is a timestamp, not `—`. `telescope:prune --hours=48` only if Telescope is installed.
